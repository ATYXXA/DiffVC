        ...
        for us_spk in dataset.unseen_speakers:
            ...
            for utt in utts:
                ...
                for tgt_spk in tgt_spks:
                    ...
                    ####
                    if uncond:
                        # torch.manual_seed(int(us_spk[1:]))
                        mel_avg, mel_rec = model(mel, mel_lengths, unit, 
                                                n_timesteps=30, hint=hint, uncond=True)      
                    else:                  
                        mel_avg, mel_rec = model(mel, mel_lengths, unit, 
                                                n_timesteps=30, hint=hint)
                    ###
                    ...
                    if eg:
                        ...
                        save_audio(f'{out_dir}/{us_spk}/{us_spk}_uc_{utt[len(us_spk)+1:]}.wav',
                                sampling_rate, audio, normalize=False)                                              
                    else:
