...
class LenPredictor(nn.Module):
    def __init__(self, n_tokens=500, emb_size=32, masking_rate=0.2, norm_mean=torch.tensor(0),
        ...
        # self.emo_proj = nn.Linear(768, emb_size)
        self.emo_proj = nn.Linear(5, emb_size)
    ...
    def forward(self, seq, emo):
        emb_seq = self.token_emb(seq)
        emb_emo = self.emo_proj(emo)
        # print(emb_seq.shape,emb_emo.shape) #torch.Size([32, 194, 32]) torch.Size([32, 32])
        ...
        emb_emo = torch.repeat_interleave(emb_emo.unsqueeze(1), seq.shape[1], dim=1)
        # print(emb_emo.shape) # torch.Size([32, 194, 32])
        emb_seq = torch.cat([emb_seq, emb_emo], dim=-1)
        # print(emb_seq.shape) # torch.Size([32, 194, 64])
