...
class UnitEncoder(BaseModule):
    def __init__(self, n_vocab, n_feats, channels, filters, heads, layers, kernel, 
        ...
        self.n_vocab = n_vocab # n_kmeans
        self.n_feats = n_feats # n_contentvec
        self.channels = channels
        self.filters = filters
        self.heads = heads
        self.layers = layers
        ...
        self.emb = torch.nn.Embedding(n_vocab, channels)
        ...
        self.proj = torch.nn.Conv1d(channels, n_feats, 1)
...
###
class UFEncoder(BaseModule):
    def __init__(self, n_vocab, n_feats, channels, filters, heads, layers, kernel, 
                 dropout, window_size=None):
        super(UFEncoder, self).__init__()
        self.n_vocab = n_vocab # n_kmeans
        self.n_feats = n_feats # n_contentvec
        self.channels = channels
        self.filters = filters
        ...
        self.emb = torch.nn.Embedding(n_vocab, channels)
        self.emb_f0 = torch.nn.Conv1d(1, channels, 1)

        self.prenet = ConvReluNorm(channels, channels, channels, 
                                   kernel_size=5, n_layers=3, p_dropout=0.5)
        self.prenet_f0 = ConvReluNorm(channels, channels, channels, 
                                   kernel_size=5, n_layers=3, p_dropout=0.5)
    ...
    def forward(self, x, x_mask, f0):
        x = self.emb(x)
        x = torch.transpose(x, 1, -1).contiguous()
        x = self.prenet(x, x_mask)
        ###
        f0 = self.emb_f0(f0)
        f0 = self.prenet_f0(f0, x_mask)
        x = x + f0
        ###
        x = self.encoder(x, x_mask)
        x = self.proj(x * x_mask)
...
class TEEncoder(BaseModule):
    def __init__(self, n_enc, n_feats, channels, filters, heads, layers, kernel, 
        ...
        self.init_proj = torch.nn.Conv1d(n_enc, channels, 1)
        self.prenet = ConvReluNorm(channels, channels, channels, 
                                   kernel_size=5, n_layers=3, p_dropout=0.5)
