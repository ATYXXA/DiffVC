...
import params
from data import VCDecBatchCollate_v3, EmotionalDataset
from model.vc import ControlledDiffVC_emo
...
if __name__ == "__main__":
    ...
    train_set = EmotionalDataset(data_dir)
    collate_fn = VCDecBatchCollate_v3()
    ...
    if ckpt_ep is not None:
        ...
        model.load_state_dict(torch.load(f'emo_from_vc43.pt'))
    ###
    iteration = 0
    for epoch in range(ckpt_ep+1 if ckpt_ep is not None else 1, epochs + 1):
        ...
        for batch in tqdm(train_loader, total=len(train_set)//batch_size):
            ...
            loss = model.compute_loss(mel, mel_lengths, unit, hint, emo)
            loss.backward()
        ...
        with torch.no_grad():
            ...
            for i, (mel, c, unit, emo) in enumerate(mels):
                if i >= test_size:
                    break
